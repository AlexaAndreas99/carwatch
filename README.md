...
Edit `config.yaml`. It is not in the repository — your search URLs are your business, and
`config.example.yaml` is the version that ships. The only fields you must fill in are the `sources[].url` values:
open each site in your browser, set your filters, run the search, then copy the URL from
the address bar and paste it in, replacing `PASTE_URL_HERE`. That URL already encodes your
filters — CarWatch just paginates it.

Any source you don't want, delete its block. A search can pull from one, two, or all three
sites. The `make` / `model` / `year_min` / `price_max` / `currency` fields are display metadata
only; the real filtering lives in the pasted URLs.

**Display-only means exactly that** — none of those fields reaches a listing. A car's currency,
year and price are whatever the adapter scraped from the site, so setting `currency: "RON"` on a
configuration does not change how its cars are counted or priced anywhere. The fields exist to
label and group a configuration, and that is all they do.

> **Nothing about the searches in `config.yaml` is baked into the code.** They happen to be
> Nissan Qashqai searches; CarWatch tracks whatever you paste in, across any subset of the
> three sites, so adding or changing a search is a config edit and never a code change. Bear in
> mind that the database is the only copy of a search's price history, and it is worth
> something only because it has been collected into repeatedly — so edit a search when your
